...
export const getMyOrders = async (req, res) => {
  try {
    ...
    // Ambil pesanan dengan detail item (nama, qty, harga)
...
export const getOrderById = async (req, res) => {
  try {
    ...
    return successResponse(res, 'Detail pesanan berhasil diambil', {
      ...order,
      customer_name: profile?.full_name?.trim() || null,
    });
  } catch (error) {
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    const enrichedOrders = orders.map(order => ({
      ...order,
      customer_name: profileMap[order.user_id]?.full_name?.trim() || null,
    }));

...
export const getAllProducts = async (req, res) => {
  try {
    ...
    // Ambil rating rata-rata untuk semua produk yang dikembalikan
    ...
    // Gabungkan rating ke setiap produk
    const productsWithRating = products.map(p => ({
      ...p,
      average_rating: ratingMap[p.id]?.average_rating || 0,
      total_reviews: ratingMap[p.id]?.total_reviews || 0,
    }));

    // Buat metadata pagination
    const pagination = getPaginationMeta(page, limit, count || 0);

    return paginatedResponse(res, 'Daftar produk berhasil diambil.', productsWithRating, pagination);
  } catch (error) {
